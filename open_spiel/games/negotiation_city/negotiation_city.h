...
// 在 new_game.h 中，重命名文件顶部和底部的标头保护。
// In the new files, rename the inner-most namespace from tic_tac_toe to new_game.
// 在新文件中，将最内部的命名空间从 tic_tac_toe 重命名为 new_game 。

#ifndef OPEN_SPIEL_GAMES_NEGOTIATION_CITY_H_
#define OPEN_SPIEL_GAMES_NEGOTIATION_CITY_H_
...
class NegotiationCityGame;

class NegotiationCityState : public State {
 public:
  NegotiationCityState(std::shared_ptr<const Game> game);
  NegotiationCityState(const NegotiationCityState&) = default;
 ...
 private:
  ...
  const NegotiationCityGame& parent_game_;
...
class NegotiationCityGame : public Game {
 public:
  explicit NegotiationCityGame(const GameParameters& params);
  explicit NegotiationCityGame(const NegotiationCityGame& other);

  int NumDistinctActions() const override;
  std::unique_ptr<State> NewInitialState() const override {
    return std::unique_ptr<State>(new NegotiationCityState(shared_from_this()));
  }
...
#endif  // OPEN_SPIEL_GAMES_NEGOTIATION_City_H_
