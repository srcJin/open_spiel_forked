...
NegotiationCityState::NegotiationCityState(std::shared_ptr<const Game> game)
    : State(game),
      parent_game_(static_cast<const NegotiationCityGame&>(*game)),
...
std::vector<int> NegotiationCityState::DecodeInteger(int encoded_value,
                                                 int dimensions,
...
int NegotiationCityState::EncodeInteger(const std::vector<int>& container,
                                    int num_digit_values) const {
  int encoded_value = 0;
...
std::vector<int> NegotiationCityState::DecodeUtterance(
...
std::vector<std::pair<Action, double>> NegotiationCityState::ChanceOutcomes()
    const {
...
std::unique_ptr<State> NegotiationCityGame::DeserializeState(
